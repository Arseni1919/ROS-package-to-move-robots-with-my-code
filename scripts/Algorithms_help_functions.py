...
def get_possible_pos_with_MR_general(self_agent, all_real_positions_for_robot):
    """
    input:
    output:
    """
    ...
    curr_pose = self_agent.get_pos()
    MR = self_agent.get_MR()
    for pos in all_real_positions_for_robot:
        x = pos[0][0]
        y = pos[0][1]
        if distance((x, y), (curr_pose[0][0], curr_pose[0][1])) < MR:
