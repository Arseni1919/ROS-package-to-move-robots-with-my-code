...
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# best_possible_local_reduction
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
def best_possible_local_reduction(temp_REQ, curr_pos, new_pos, SR, my_cred):
    def get_max_val(pos):
        max_val = 0
        for target in temp_REQ:
            if in_range(pos, target, SR) and max_val < target[2]:
                max_val = target[2]
        return max_val

    curr_cov = get_max_val(curr_pos)
    new_cov = get_max_val(new_pos)
    pos = curr_pos if curr_cov > new_cov else new_pos
    print 'the LR is: %s and the position of LR is: %s' % (min(new_cov - curr_cov, my_cred), pos)
    return min(new_cov - curr_cov, my_cred), pos
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
...
def collect_positions_from_neighbours():
    ...
    while 'None' in dict_neighbours_positions.values():
        rate.sleep()
    print 'dict_neighbours_positions (real): %s' % dict_neighbours_positions

    while 'None' in dict_end_of_movements.values():
        rate.sleep()
    print 'dict_end_of_movements: %s' % dict_end_of_movements
...
def collect_improvements_from_neighbours():
    global dict_neighbours_improvements
    while 'None' in dict_neighbours_improvements.values():
        rate.sleep()
    print 'dict_neighbours_improvements: %s' % dict_neighbours_improvements


def get_reachable_positions(num_robot, curr_pos, MR):
    all_positions = vars.robots_possible_positions.get(num_robot)
...
def callback_LR(num_robot, LR, y='0'):
    global dict_neighbours_improvements
    dict_neighbours_improvements[num_robot] = float(LR)
...
if __name__ == '__main__':
    ...
    curr_pos = vars.robots_start_positions.get(num_of_robot)
    dict_neighbours_positions = {}
    ...
    print "All subs, pubs, vars and clients are in action in robot %s " % num_of_robot
    ...
    while iteration < vars.num_of_iterations:
        ...
        dict_neighbours_improvements = {key: 'None' for key in vars.neighbours.get(num_of_robot)}
        ...
        possible_positions = get_reachable_positions(num_of_robot, curr_pos, vars.real_MR)
        temp_REQ = get_temp_REQ(vars.real_targets, vars.real_SR)
        new_pos = select_pos(possible_positions, temp_REQ, vars.real_SR)

        # best_possible_local_reduction returns tuple --> (lr, pose)
        LR, LR_pose = best_possible_local_reduction(temp_REQ, curr_pos, new_pos, vars.real_SR,
                                                    vars.credibility[num_of_robot])

        print("robot %s sending improvements" % num_of_robot)
        rate.sleep()  # in order to wait to others
        publish(num_of_robot, 'LR', LR)
        collect_improvements_from_neighbours()

        if LR > 0:
            to_change = True
            # filter:
            for neighbour, lr in dict_neighbours_improvements.iteritems():
                if LR < lr:  # by value
                    to_change = False
                    break
                if LR == lr:
                    if int(num_of_robot) > int(neighbour):  # by index
                        to_change = False
                        break
            if to_change:
        ...
        print "robot %s's position in the the end of %s iteration is %s " % (num_of_robot, iteration, curr_pos)
        publish(num_of_robot, 'end_of_movement')

        iteration += 1
        rate.sleep()

    print "Finished MGM_MST"
